...
meqmaker.add_sky_jones(oms_dipole_projection);
...
def _define_forest (ns):
  ...
  Meow.StdTrees.vis_inspector(ns.inspector('output'),output,
                              bookmark="Inspect output visibilities");
  inspectors.append(ns.inspector('output'));
  Meow.Bookmarks.make_node_folder("Output visibilities by baseline",
    [ output(p,q) for p,q in array.ifrs() ],sorted=True,ncol=2,nrow=2);
