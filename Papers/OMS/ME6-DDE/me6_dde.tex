...
Thus, given a known but unpolarized sky, we can only determine $\jones{G}{p}$ to within an arbitrary unitary ambiguity factor $\jones{U}{}$. In other words, {\em we cannot fix the polarization response of our system without polarized calibrators.} 

As it turns out, even a polarized calibrator {\em alone} is insufficient, though the matrix algebra gets a bit complicated at this point. Because the $\coh{B}{}$ matrix is Hermitian positive-definite by construction,\footnote{This follows from $I \ge Q+U+V,$ but the full proof is somewhat laborious.} it has a Cholesky decomposition $\coh{L}{}\coh{L}{}^H=\coh{B}{}$. For any unitary $\jones{U}{}$, we then have:
...
Therefore, given a single polarized calibrator, we still have an ambiguity factor of 
$\coh{L}{} \jones{U}{} \coh{L}{}^{-1}$! The upshot of this is that full polarization calibration of an interferometer requires either {\em multiple} calibrator sources, and/or additional assumptions about the sky (e.g. $V=0$, which was a common assumption in the pre-RIME era). \citet{ME5} explores these issues in more detail.
